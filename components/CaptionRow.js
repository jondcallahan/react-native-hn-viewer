...
export const CaptionRow = ({relDate, upvotes, numComments, id, index, navigation}) => {
  return (
    <View style={{display: 'flex', flexDirection: 'row', marginVertical: 2}}>
      <Caption1>{relDate}</Caption1>
      {(Number.isInteger(upvotes) && (
        <Caption1 style={{flex: 1, marginLeft: 8}}>⬆️ {upvotes}</Caption1>
      )) || (
      ...
      {numComments && (
        <TouchableOpacity
          ...
          onPress={() => {
            navigation.navigate('Child', {id, index});
          }}
        >
